...
## 7. 自動スモークテスト (2026-02-15 追加)

手動検証の前段として、以下のE2Eスモークテストを追加しました。

- 実行コマンド: `npm run test:e2e-smoke`
- 対象スクリプト: `tests/e2e-offline-smoke.mjs`
- 検証項目:
  - Service Workerキャッシュ完全性
  - センサー欠損時のステータス遷移と復帰
  - 設定保存失敗時のToast通知
  - オフライン起動
  - 外部フォントURLへのアクセスなし

詳細な実行結果は `docs/high-precision-sensor/testing/TS-02_AutomatedVerificationReport.md` を参照してください。
